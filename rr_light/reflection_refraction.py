...
def check_invalid_alfa(alfa: float) -> None:
    ...
    :raises ValueError: in case of invalid alfa.
    """
...
def check_invalid_index_of_refr(n1: float, n2: float) -> None:
    ...
    :raises ValueError: in case of invalid refl. indices.
    """
...
def calculate_refr_angl(
    alfa: float, n1: float, n2: float
    ...
    if is_tot_intern_refl(alfa, n1, n2):
        ...
        try:
            beta = math.degrees(
                math.asin(n1 * math.sin(math.radians(alfa)) / n2)
            )  # n1*sin(θ1) = n2*sin(θ2)
        except ValueError:
            beta = None
...
def matplotlib_graph(
    p1: List[float], p2: List[float], p3: List[float], n1: float, n2: float
) -> str:
    ...
    plt.arrow(
        ...
        dx=DISTANCE * -p1[0],
        dy=DISTANCE * -p1[1],
        facecolor=COLOR_OF_INCIDENT_BEAM,
        width=0.4,
        head_width=1,
        head_length=1.5,
    ...
    plt.arrow(
        ...
        dx=DISTANCE * p2[0],
        dy=DISTANCE * p2[1],
        facecolor=COLOR_OF_REFLECTED_AND_REFRACTED_BEAM,
        width=0.4,
        head_width=1,
        head_length=1.5,
        length_includes_head=True,
